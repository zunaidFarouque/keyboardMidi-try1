...
void VoiceManager::noteOn(InputID source, const std::vector<int> &notes,
                          ...
                          int strumSpeedMs, bool allowSustain) {
  if (notes.empty())
    return;
  ...
  // Ensure velocities vector matches notes size (use first velocity as default
  // if needed)
  ...
  if (strumSpeedMs == 0) {
    ...
    for (size_t i = 0; i < notes.size(); ++i) {
      ...
      midiEngine.sendNoteOn(channel, notes[i],
                            static_cast<float>(vel) / 127.0f);
      voices.push_back(
          {notes[i], channel, source, allowSustain, VoiceState::Playing});
    }
  } else {
...
void VoiceManager::hiResTimerCallback() {
  ...
  while (it != pendingReleases.end()) {
    ...
    if (now >= expirationTime) {
      // Release duration expired - send noteOff to all voices from this source
      // (Normal mode)
