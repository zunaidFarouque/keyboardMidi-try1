...
public:
  ...
  // Generate chord notes (root position only). For piano/guitar voicing use
  // generateChordForPiano / generateChordForGuitar.
  static std::vector<ChordNote>
  generateChord(int rootNote, const std::vector<int> &scaleIntervals,
  ...
  // guitarFretAnchor+3. Returns 4-6 ChordNotes (no ghost); bass isolation
  // applied (mute low E when root on A, etc.).
  static std::vector<ChordNote>
  generateChordForGuitar(int rootNote, const std::vector<int> &scaleIntervals,
                         int degreeIndex, ChordType type, int fretMin,
                         int fretMax);
...
private:
  ...
  static std::vector<int> applyGravityWell(const std::vector<int> &notes,
                                           int centerPitch);
  // Alternating Grip: odd degree -> root position, even degree -> 2nd inversion
  // (7th/9th)
  static std::vector<int> applyAlternatingGrip(const std::vector<int> &notes,
                                               int degreeIndex);
  ...
  static std::vector<int> applySmartFlow(const std::vector<int> &notes,
                                         ChordType type, int degreeIndex,
                                         int centerPitch);
  // Helper: Add ghost notes to fill gaps in voicing
  static void addGhostNotes(std::vector<ChordNote> &notes, int zoneAnchor,
                            ChordType type,
